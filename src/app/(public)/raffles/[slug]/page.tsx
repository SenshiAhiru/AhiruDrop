            ...
            <div className="space-y-6">
              ...
              {raffle.status === "DRAWN" && winnerInfo && (
                <div className="rounded-2xl border border-accent-500/40 bg-gradient-to-br from-accent-500/20 via-accent-500/5 to-transparent p-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-sm font-bold text-accent-400 uppercase tracking-wider">{t("raffleDetail.statusDrawnPill")}</p>
                  </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-[var(--muted-foreground)]">{t("raffleDetail.raffleProgress")}</span>
                  <span className="font-semibold text-[var(--foreground)]">{paidPercentage}%</span>
                </div>
                <Progress value={paidPercentage} />
                <div className="flex items-center justify-between text-sm text-[var(--muted-foreground)]">
                  <span>{t("raffleDetail.quotasSold", { sold: raffle.stats.paid, total: raffle.totalNumbers })}</span>
                  <span>{t("raffleDetail.quotasAvailable", { n: raffle.stats.available })}</span>
                </div>
              </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-emerald-300">{t("raffleDetail.provablyFair")}</p>
                  <p className="text-xs text-emerald-400/70">{t("raffleDetail.provablyFairSub")}</p>
                </div>
              ...
              {raffle.scheduledDrawAt && (
                <div className="flex items-center gap-3 rounded-xl border border-accent-500/30 bg-accent-500/5 p-4">
                  ...
                  <div>
                    <p className="text-xs text-[var(--muted-foreground)]">{t("raffleDetail.drawInShort")}</p>
                    <p className="text-lg font-bold font-mono text-accent-400">
                      {countdown || t("raffleDetail.drawSoon")}
                    </p>
                  </div>
                </div>
              )}
